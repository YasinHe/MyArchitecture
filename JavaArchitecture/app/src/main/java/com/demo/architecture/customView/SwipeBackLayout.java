package com.demo.architecture.customView;
...
public class SwipeBackLayout extends ViewGroup {
    ...
    private int draggingOffset;//关闭页面距离

    //控制屏幕手势滑动距离(只是左往右滑动判断)
    private int swipeSize = 80;
    public void setSwipeSize(int size){
        swipeSize = size;
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if(ev.getRawX() > swipeSize&&(dragEdge == DragEdge.LEFT)){
            return handled;
        }
        ensureTarget();
        if (isEnabled()) {////只有触摸屏幕边缘才有用
